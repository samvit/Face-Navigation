...
main(int argc, char** argv)
{
  ...
  //   printf("\nUnable to Launch JVM\n");
    ...
    cout << "while loop";
      ...
      // cls = env->FindClass("MouseClicker");
      // cout << cls;
      
      char strOut[21]; // enough to hold all numbers up to 64-bits
      sprintf(strOut, "%d", justBlinked);
      
      DRAW_TEXT(frame, strOut, text_delay, 1);
      
      /* if(cls !=0) {
        DRAW_TEXT(frame, "inIF", text_delay, 1);
        mid = env->GetStaticMethodID(cls, "mouseClick", "()V");
        cout << "in cls!=0";
        if (mid != 0 ) {
        ...
        DRAW_TEXT(frame, "regular", text_delay, 1);
      } */
    }
